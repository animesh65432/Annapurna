...
export default function MobileSearch({ diet, cuisine, q }: Props) {
    ...
    const DishTypeOptions = t("explore.Diet_Type_Options", { returnObjects: true }) as string[];
    const CuisineOptions = t("explore.CuisineOptions", { returnObjects: true }) as string[];
    const handleSearch = () => {
                        ...
                        <div className="flex  flex-col sm:flex-row w-full sm:w-[90%] h-[10vh] ml-auto mr-auto gap-5 sm:gap-2">
                            <Select onValueChange={(value) => SetDietType(value)} >
                                ...
                                <SelectContent >
                                    {DishTypeOptions.map((dish) => {
                                        const [key, label] = Object.entries(dish)[0];
                            ...
                            <Select onValueChange={(value) => setCuisine(value)}>
                                ...
                                <SelectContent className="">
                                    {CuisineOptions.map((place) => {
                                        const [key, label] = Object.entries(place)[0];
                                        return <SelectItem key={key} value={key}>{label}</SelectItem>
                                    })}
                                </SelectContent>
                            </Select>
